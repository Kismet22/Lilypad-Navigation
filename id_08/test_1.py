...
from env_new import train_env_obstacle
# Env Obstacle Position & Collision Mark
from env_new import train_env_obstacle_1
# Env Obstacle Position
from env_new import train_env_filed
# Env Obstacle Position Quadrant
from env_new import train_env_fakeobt
# Env Obstacle Roll Guide
...
# roll guide
# ppo_path = './models/normal_task/0427_4.pth'
ppo_path = './models/normal_task/0506_1.pth'
# ppo_path = './models/normal_task/0508_0.pth'
# ppo_path = './models/normal_task/0512_6.pth'
# normal
# ppo_path = './models/normal_task/0427_12.pth'
...
def success_rate_test_1(_seed=1):
    ...
    #                     delta_t=env.dt, action_std_init=action_std_4_test,
    ...
    # success_but_collided_trajectories = []
    # failed_trajectories = []
    # failed_and_collided_trajectories = []
    # Test
...
def success_rate_test(_id_in=1):
    ...
    print(colored(f"**** Success: {success_times} ****", 'green'))
    print(colored(f"**** OutBound: {outbound_times} ****", 'red'))
    print(colored(f"**** TimeOut: {timeout_times} ****", 'red'))
    print("Average Navigation Time:", average_time)
    # name of ppo method
    ppo_name = os.path.splitext(os.path.basename(ppo_path))[0]
    success_name = f"{ppo_name}_success_{_id}.csv"
    collide_name = f"{ppo_name}_collide_{_id}.csv"
    outbound_name = f"{ppo_name}_outbound_{_id}.csv"
    timeout_name = f"{ppo_name}_timeout_{_id}.csv"
    success_filename = os.path.join(save_dir, success_name)
    _save_trajectories(file_name=success_filename, _trajectories=success_trajectories)
    collide_filename = os.path.join(save_dir, collide_name)
    _save_trajectories(file_name=collide_filename, _trajectories=collide_trajectories)
...
def test():
    ...
    _start = np.array([float(220), float(64)])
    _target = np.array([float(64), float(64)])
    env = train_env_fakeobt.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed, _pos_normalize=True)
    # env = train_env_filed.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed)
    # env = train_env_separate.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed, _pos_normalize=True)
    # env = train_env_path_follow.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed)
    # env = train_env_obstacle.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56)

    # fixed point
    # _start = np.array([float(180.77), float(43.55)])
    # _target = np.array([float(16.35), float(43.55)])
    # _start = np.array([float(208.4), float(97.1)])
    # _target = np.array([float(23.3), float(48.2)])
    # env = train_env_fakeobt.foil_env(args_1, max_step=max_steps, start_position=_start, target_position=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed, _pos_normalize=True)
    # env = train_env_separate.foil_env(args_1, max_step=max_steps, start_position=_start, target_position=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed)
    # env = train_env_path_follow.foil_env(args_1, max_step=max_steps, start_position=_start, target_position=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56, _init_flow_num=random_seed)
    # env = train_env_obstacle.foil_env(args_1, max_step=max_steps, start_position=_start, target_position=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False, _random_range=56)

    # Old Env
    # _target = np.array([float(64), float(96)])
    # _start = np.array([float(220), float(32)])
    # env = flow_field_env_2_1.foil_env(args_1, max_step=max_steps, target_position=_target, start_position=_start, include_flow=True)
    ######################################################
    ...
    for i in range(1, max_ep_len + 1):
        ...
        # state, reward, terminated, truncated, _, info = env.step(action)
