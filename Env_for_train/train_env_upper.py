...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _state_dim=18, u_range=20.0, v_range=20.0, w_range=5.0, _forward_only = True, _obstacle_avoid=True, _is_switch=False):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        if _forward_only:
            ...
            # self.action_dim = 3
            # low = np.array([-1.0, -1.0, 6], dtype=np.float32)
            # high = np.array([1.0, 1.0, 12], dtype=np.float32)
        self.low = low
        self.high = high
