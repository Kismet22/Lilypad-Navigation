...
class foil_env:
    ...
    def get_flow_velocity(self):
        """
        Compute the velocity field around the agent with:
        - A (2*flow_range x 2*flow_range) square region
        - Right edge aligned at agent_pos + a
        - Mask remains a (2a x 2a) square at agent_pos
        """
        ...
        safety_margin = 4
        pad_width = 2 * r_large + safety_margin

        v_x_padded = np.pad(v_x, pad_width=pad_width, mode='reflect')
        v_y_padded = np.pad(v_y, pad_width=pad_width, mode='reflect')

        # Compute aligned window edges:
        # pos x range
        win_size = 2 * r_large
        y_right = y + a
        y_left = y_right - win_size + 1
        y_center = (y_left + y_right) / 2

        x_padded = int(x + pad_width)
        y_padded_center = int(y_center + pad_width)

        # Correct slicing to get exactly (2*r_large, 2*r_large) output while preserving alignment
        v_x_region = v_x_padded[x_padded - r_large : x_padded + r_large,
                                y_padded_center - r_large : y_padded_center + r_large]
        v_y_region = v_y_padded[x_padded - r_large : x_padded + r_large,
                                y_padded_center - r_large : y_padded_center + r_large]

        # Apply mask at agent_pos
        mask = np.ones_like(v_x_region, dtype=bool)
        center_i = r_large
        center_j = r_large + int(y - y_center)  # relative offset from the region's center to agent_pos
        mask[center_i - a : center_i + a,
            center_j - a : center_j + a] = False

        v_x_region[~mask] = 0
        v_y_region[~mask] = 0

        return v_x_region, v_y_region
        ...
        #####################################################################################################
            ...
            #####################################################################################################
                ...
                if self.observation_dim == 16:
                    _reward = -50 * self.dt + target_reward + 1.6 * avoid_penalty
                ########################
                ...
                if full_turns != 0:
                    ...
                    # === too many rolls ===
                    if self._is_test:
                        max_roll_threshold = 1
                    else:
                        max_roll_threshold = 5
                    if self._rolls >= max_roll_threshold:
                        print(colored(f"**** Episode terminated: Over-Roll At Step {self.step_counter} ****", 'red'))
                        _truncated = True
                        _reward = -500
