...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
        ...
        self.target_position_end = self.target_default
        self.start_position = self.start_default
        self.cloest_obstacle = np.array([108, 64])
        ...
        if target_position is not None:
            ...
            self.target_position_end = target_position
        else:
            # Random Select
    ...
    def reset_target(self):
        self.target_position = self.target_position_end.copy()
    ...
    def step(self, action):
        ...
        d_1 = np.linalg.norm(self.agent_pos - self.target_position_end)
        ########################################################################################
        ...
        if len(_barricade_states) == 0:
            ...
            cloest_obstacle_center =  np.array(_barricade_states[0][4], dtype=np.float32)
            self.cloest_obstacle = cloest_obstacle_center
            agent_pos = np.array(self.agent_pos, dtype=np.float32)
            target_pos = np.array(self.target_position_end, dtype=np.float32)
    ...
    def reset(self):
        ...
        if not self._is_fixed_target:
            ...
            self.target_position_end = self.target_position.copy()
    ...
    def plot_env(self, ax, sample_rate=10):
        ...
        target = self.target_position_end
        ...
        # Current Target
        ax.scatter(*current_target, color='red', label='Target Point', zorder=5)
