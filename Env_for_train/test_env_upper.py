...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _plot_flow=False, _proccess_flow=False, _random_range = 56, _flow_range=16, _init_flow_num=0, _pos_normalize=True, _is_test=False, _state_dim=18):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        lc_low = np.array([-20.0, -20.0, -5.0], dtype=np.float32)
        lc_high = np.array([20.0, 20.0, 5.0], dtype=np.float32)
        self.lc_low = lc_low
        ...
        #####################################################################################################
            ...
            #####################################################################################################
                ...
                if self.observation_dim == 16:
                    _reward = -10 * self.dt + target_reward + avoid_penalty
                ...
                if self.observation_dim == 22:
                    _reward = -10 * self.dt + target_reward + avoid_penalty + angle_reward + border_penalty + roll_swich_punish 
                ...
                if full_turns != 0:
                    ...
                    if self._rolls > max_roll_threshold:
                        print(colored("**** Episode terminated: Over-Roll ****", 'red'))
                        _truncated = True
                        _reward = -500
