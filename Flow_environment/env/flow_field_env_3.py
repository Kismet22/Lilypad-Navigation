...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None, target_position=None, start_position=None, max_step=None, include_flow=False):
        ...
        low = np.array([-0.00001, -0.00001, -pi], dtype=np.float32)  # 每个维度的下界
        high = np.array([0.00001, 0.00001, pi], dtype=np.float32)  # 每个维度的上界
        self.low = low
        self.high = high
        ...
        self.target_angle_range = [pi/6, pi/4]            
        self.reward = 0
        self.d_2_target = 0
        self.a_2_target = 0
        self.angle = 0
