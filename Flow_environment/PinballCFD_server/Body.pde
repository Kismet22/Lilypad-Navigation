...
class Body {
  ...
  void react (BDIM flow, PVector force, float moment) {
    PVector f = pressForce(flow.p).mult(-1);
    float m = pressMoment(flow.p)*(-1);
    PVector f_global = bodyToGlobalForce(force);
    PVector f_total = f_global.add(f);
    float m_total = m + moment;
    react(f_total,m_total,flow.dt);
  }
  ...
  PVector bodyToGlobalForce(PVector bodyForce) {
    float sa = sin(phi), ca = cos(phi);
    return new PVector(
      bodyForce.x * ca - bodyForce.y * sa,  // Fx = Ft*cos(φ) - Fn*sin(φ)
      bodyForce.x * sa + bodyForce.y * ca   // Fy = Ft*sin(φ) + Fn*cos(φ)
    );
  }

  ArrayList<Float> calculateSurfacePressures(Field p) {
    ArrayList<Float> pressures = new ArrayList<Float>();
    int count = 0;
    for (OrthoNormal o : orth) {
        if (count % 5 == 0) { // 每隔5个点添加一次压力
            float pressure = p.linear(o.cen.x, o.cen.y);
