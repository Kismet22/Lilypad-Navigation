...
def train(model_id, render_mode=None, checkpoint_path=None):
    ...
    state_dim_mode = 6
    # state_dim_mode = 11
    # env_name = f'Flow_env_Obstacle_new_state_dim_{state_dim_mode}'
    env_name = f'RL_X_V_P_env_state_dim_{state_dim_mode}'
    ...
    env = train_env_basic_2.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _random_range=_random_range, _switch_range=_switch_range, 
    _init_flow_num=flow_seed, _pos_normalize=_is_normalize, _state_dim=state_dim_mode, 
    _is_random = true_random, _set_random=random_seed, _is_test = test_mode, _is_switch = switch_mode, u_range=acc_max, v_range=acc_max, u_clip=acc_max, v_clip=acc_max)
