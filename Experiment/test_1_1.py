...
ppo_path = './models/09/rl_dim2_a15_normal_notest.pth' # icm
# easy
...
def test():
    ...
    video_dir = "./Video_rl_normal/video_frames"

    # random point
    _start = np.array([float(220), float(64)])
    _target = np.array([float(64), float(64)])
    env = train_env_basic_2_2.foil_env(args_1, max_step=max_steps, start_center=_start, target_center=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False,  _swich_range=switch_range, _random_range=56, 
    _init_flow_num=random_seed, _pos_normalize=True, _is_test=True, _state_dim=state_dim_mode, plot_p=False, video_dir=video_dir, u_range=15.0, v_range=10.0, w_range=5.0, u_clip=15.0, v_clip=10.0, w_clip=5.0)
    
    # fixed point
    # _start = np.array([float(240), float(32)])
    # _target = np.array([float(45.64638910502006), float(57.540054068652765)])
    # _start = np.array([float(173), float(81)])
    # _target = np.array([float(19.3), float(33.3)])
    ###################################################

    ###################################################
    # env = train_env_basic_2_2.foil_env(args_1, max_step=max_steps, start_position=_start, target_position=_target, _include_flow=True, _plot_flow=True, _proccess_flow=False,  _swich_range=switch_range, _random_range=56, 
    # _init_flow_num=random_seed, _pos_normalize=True, _is_test=True, _state_dim=state_dim_mode, plot_p=False, video_dir=video_dir, u_range=15.0, v_range=10.0, w_range=5.0, u_clip=15.0, v_clip=10.0, w_clip=5.0)
    ######################################################
