...
def train(model_id, render_mode=None, checkpoint_path_high=None, checkpoint_path_low=None):
    ...
    if _is_head:
        ...
        if theta_mode:
            ...
            if not _obstacle_mode:
                ...
                env_name = f'Flow_env_Upper_dim_{state_dim_mode}_roll'
    ...
    _forward_only=_is_head, _obstacle_avoid=True, _theta_mode = theta_mode, _is_switch=switch_mode, u_clip=clip_max, v_clip=clip_max, _near_mode=_obstacle_mode)
    ...
    ################################################################################################################## 
    ################# Init ################
    if checkpoint_path_high or checkpoint_path_low:
        print("============================================================================================")
        print("Loading......")
    ...
    # while hl_time_step <= hl_max_training_timesteps:
        ...
        while not done and t < max_ep_len:
            ...
            if _is_head:
                lc_target_x, lc_target_y = compute_subgoal_forward(pos=agent_position, angle=high_action, r=3)
            else:
                if not theta_mode:
                    vec = high_action
                ...
                lc_target_x, lc_target_y = compute_subgoal(pos=agent_position, angle=high_action, r=3)
            ...
            # === 混合训练：根据子目标的接近程度 ===
            subgoal_reward = 10 * max((relative_dist_to_subgoal - dist_to_subgoal), -2)
            if lc_train:
                cumulative_high_reward += subgoal_reward
            ...
            #     cumulative_high_reward += subgoal_reward
            # else:
            #     progress = time_step / max_training_timesteps
            #     # weight = np.exp(-10 * progress)
            #     cutoff = 0.3
            #     weight = 1 - (progress / cutoff) if progress < cutoff else 0.0
            #     cumulative_high_reward += weight * 0.2* subgoal_reward
            # current_ep_return += cumulative_high_reward
