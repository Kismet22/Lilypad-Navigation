...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _test_info=False, video_dir="./hrl_model_output/video_frames", _obstacle_mode=False):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        self._obstacle_mode = _obstacle_mode
        #################################################
    ...
    def step(self, action):
        ...
        # 感知范围 (全局下): 半径 + 扇区 (可选)
        _fov = np.pi   # 默认180° (可改成2π表示全向)
        if self._obstacle_mode:
            _fov = 2.0 * np.pi

        # 全局参考方向 = x轴负方向
        reference_dir = np.array([-1.0, 0.0], dtype=np.float32)  

        _barricade_states = []

        for circle in self.circles[:3]:
            obstacle_pos = np.array(circle["center"], dtype=np.float32)
            obstacle_radius = circle.get("radius", 1.0)
            ...
            # 全局范围筛选：是否在扇区内
            unit_vec_edge = (edge_point_world - agent_pos) / (edge_distance + 1e-8)
            dot = np.dot(reference_dir, unit_vec_edge)
            ...
            _barricade_states.append((
                edge_distance,
                edge_point_world[0] - agent_pos[0],  # dx (全局)
                edge_point_world[1] - agent_pos[1],  # dy (全局)
                edge_point_world
            ))
        ...
        if len(_barricade_states) == 0:
            ...
            closest_dx, closest_dy = (self.max_detect_dis, self.max_detect_dis) if not self._pos_normalize else (1.0, 1.0)
            direction = 0
            angle_rad = np.pi
        else:
            ...
            if self._pos_normalize:
                closest_dx = np.clip(_barricade_states[0][1] / self.max_detect_dis, -1.0, 1.0)
                closest_dy = np.clip(_barricade_states[0][2] / self.max_detect_dis, -1.0, 1.0)
            else:
            ...
            #####################################################################################################
                ...
                if self.observation_dim == 18 or self.observation_dim == 10:
                    if self._obstacle_avoid:
                        if self._obstacle_mode:
                            _reward = 0.5 * time_punish + target_reward + avoid_penalty + angle_reward + roll_swich_punish
                        else:
