...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _plot_flow=False, _proccess_flow=False, _random_range = 56, _switch_range = 24, _flow_range=16, _init_flow_num=0, _pos_normalize=True, 
             ...
             u_range=20.0, v_range=20.0, w_range=5.0, u_clip=20.0, v_clip=20.0, w_clip=5.0):
        """
        Basic Info:
        - Expansion Factor: 16
    ...
    def step(self, action):
        ...
        #####################################################################################################
            ...
            #####################################################################################################
                ...
                if self.observation_dim in (10, 18):
                    _reward += avoid_penalty + angle_reward + roll_swich_punish
                elif self.observation_dim in (3, 6, 11, 14):
                    pass
                elif self.observation_dim == 16:
                    _reward += 1.6 * avoid_penalty
                else:
                    raise ValueError(f"Unknown observation_dim: {self.observation_dim}")
                ########################
