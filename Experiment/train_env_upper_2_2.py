...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _plot_flow=False, _proccess_flow=False, _random_range = 56, _flow_range=16, _swich_range = 12, _init_flow_num=0, _pos_normalize=True, _is_test=False, _is_random = True, _set_random=0,
             ...
             _test_info=False, video_dir="./hrl_model_output/video_frames", _near_mode=False, _fixed_test=False):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        self._near_mode = _near_mode
        self._roll_center = np.array([0.0, 0.0])
        ...
        ############################
        # self.max_detect_dis = 48
        # self._old_dis_2_barricade = 48
    ...
    def step(self, action):
        ...
        # Field of view (这里我保留全向 2π，你也可以改成 np.pi)
        # _fov = 2 * np.pi
        _fov = np.pi
        ...
        if self._near_mode:
            # 绕行模式：直接使用固定障碍物
            obstacle_pos = np.array(self._roll_center, dtype=np.float32)
            obstacle_radius = self.circles[0].get("radius", 1.0)

            vec_world = obstacle_pos - agent_pos
            center_distance = np.linalg.norm(vec_world)
            edge_distance = max(center_distance - obstacle_radius, 1e-5)  # 保证正数
        ...
        if len(_barricade_states) == 0:
            ...
            if self._pos_normalize:
                ...
                closest_dx = raw_dx
                closest_dy = raw_dy
                _dis_2_barricade = closest_distance
            ...
            angle_rad = np.arccos(np.clip(dot, -1.0, 1.0)) #[0, pi]
            # direction = np.sign(cross)

            eps = 1e-6
            direction = np.sign(cross) if abs(cross) > eps else np.sign(eps)
        ...
        # Too Far From Center state: outbound
        elif self._near_mode and _dis_2_barricade > 48:
            _out_bound = True
            _truncated = True
            _reward = -100
            print(colored(f"**** Episode Finished At: {self.step_counter} **** Too Far.", 'red'))
        #####################################################################################################
        else:
            #####################################################################################################
            ...
            #####################################################################################################
                ...
                if self._near_mode:
                    ...
                    angle_reward = 2 * (self.old_angle - angle_norm) * abs(self.old_direction) * abs(direction)
                    roll_swich_punish = -100 * abs(direction - self.old_direction) * abs(self.old_direction) * abs(direction)
                else:
                    avoid_penalty = 25 * (_dis_2_barricade - self._old_dis_2_barricade)
                    target_reward = 10 * (self.d_2_target - d)
                    angle_reward = 50 * (self.old_angle - angle_norm) * abs(self.old_direction) * abs(direction)
                    roll_swich_punish = -100 * abs(direction - self.old_direction) * abs(self.old_direction) * abs(direction)
                ...
                if self.observation_dim == 14 or self.observation_dim == 6:
                    _reward = -2 * self.dt + target_reward
                if self.observation_dim == 16 or self.observation_dim == 8:
                    _reward = -2 * self.dt + target_reward + avoid_penalty
                if self.observation_dim == 18 or self.observation_dim == 10:
                    if self._obstacle_avoid:
                        if self._near_mode:
                            _reward = -2 * self.dt + target_reward + angle_reward + roll_swich_punish
                            min_dist = self.max_detect_dis
                            max_dist = 2 * self.max_detect_dis
                            if _dis_2_barricade < min_dist:
                                # 距离绕行中心太近
                                # _reward -= 25 * (min_dist - _dis_2_barricade)
                                _reward += avoid_penalty
                            elif _dis_2_barricade > max_dist:
                                # 距离绕行中心太远
                                # _reward -= 25 * (_dis_2_barricade - max_dist)
                                _reward -= avoid_penalty
                        else:
                            _reward = -2 * self.dt + target_reward + avoid_penalty + angle_reward + roll_swich_punish
                    else:
                        _reward = -2 * self.dt + target_reward + border_penalty
                if self.observation_dim == 20:
                    _reward = -2 * self.dt + target_reward + avoid_penalty + border_penalty  
                if self.observation_dim == 22:
                    _reward = -2 * self.dt + target_reward + 2 * avoid_penalty + angle_reward + roll_swich_punish + border_penalty 
                if self.observation_dim == 25:
                    _reward = -2 * self.dt + target_reward + avoid_penalty + angle_reward + border_penalty + roll_swich_punish 
                    if d < near_threshold:
                        _reward = -2 * self.dt + 5 * target_reward
                ########################
        ...
        if self.observation_dim == 6:
            # no pressure
            self.state = state_1[:6]
        if self.observation_dim == 8:
    ...
    def generate_start_and_target_pair_through_obstacle(self, side_buffer=8.0, min_offset_deg=10, max_offset_deg=45, max_attempts=100):
        """
        生成一对起点和目标点，使直线穿过障碍物，保证最小安全距离且离边界足够远
        每个障碍物尝试多次，如果失败换下一个障碍物（随机顺序，不重复选择）
        :param side_buffer: 离边界最小距离
        :param min_offset_deg: 偏差最小角度（度）
        :param max_offset_deg: 偏差最大角度（度）
        :param max_attempts: 单个障碍物最大尝试次数
        :return: start_pos, target_pos (numpy array)
        """
        x_min, x_max = side_buffer, self.x_range - side_buffer
        y_min, y_max = side_buffer, self.y_range - side_buffer

        # 随机打乱障碍物顺序，保证不重复
        obstacles = random.sample(self.circles, len(self.circles))

        for obstacle in obstacles:
            cx, cy = obstacle["center"]
            obstacle_radius = self.window_r / 2
            max_detect_dis = self.max_detect_dis

            effective_r_min = max_detect_dis + 8
            effective_r_max = max_detect_dis + 12

            for attempt in range(max_attempts):
                # 右侧起点
                theta_start = random.uniform(-np.pi/2, np.pi/2)
                r_start = random.uniform(effective_r_min, effective_r_max)
                x_start = cx + r_start * np.cos(theta_start)
                y_start = cy + r_start * np.sin(theta_start)
                if not (x_min <= x_start <= x_max and y_min <= y_start <= y_max):
                    continue

                # 左侧目标，顺/逆时针偏移
                offset_deg = random.uniform(min_offset_deg, max_offset_deg)
                clockwise = random.choice([True, False])
                if clockwise:
                    theta_target = theta_start + np.pi + np.deg2rad(offset_deg)
                else:
                    theta_target = theta_start + np.pi - np.deg2rad(offset_deg)
                theta_target = (theta_target + np.pi) % (2 * np.pi) - np.pi

                r_target = random.uniform(effective_r_min, effective_r_max)
                x_target = cx + r_target * np.cos(theta_target)
                y_target = cy + r_target * np.sin(theta_target)
                if not (x_min <= x_target <= x_max and y_min <= y_target <= y_max):
                    continue
        ...
        # 所有障碍物尝试完仍失败
        raise RuntimeError("No legal start ot target")

    def reset(self):
        while True:  # 循环直到成功生成起终点
            try:
                self.safe_range_flag = False
                self.frame_id = 0
                ########################################################################################
                ...
                if self._near_mode:
                    ...
                    # 可能抛出异常
                    self.start_position, self.target_position = self.generate_start_and_target_pair_through_obstacle()

                # 如果执行到这里没有抛异常，说明成功生成起终点
                break

            except RuntimeError as e:
                print(f"[Reset Warning] reset: {e}")
                continue
    ...
    #     if _save:
    #         save_path = './hrl_model_output/path.png'
    ...
    def plot_env(self, ax, sample_rate=10):
        ...
        if self._obstacle_avoid:
            ...
            start_default = [240, 64]
            target_default = [160, 64]
        ...
        # Add colorbar for background heatmap (only once)
        if not hasattr(ax, "_speed_colorbar_added"):
            plt.colorbar(im, ax=ax, fraction=0.05, label="Flow Speed (m/s)")
            ax._speed_colorbar_added = True

        # flow direction
        # Determine sampling rate based on target number of arrows
        desired_num_arrows_x = 32
        ...
        # Plot normalized direction arrows with color representing speed
        arrow = ax.quiver(
            X_sample, Y_sample,
            norm_fx, norm_fy,
            color='black',
        ...
        # Switch Range
        # ax.add_patch(plt.Circle(target, self.switch_range, color='magenta', fill=False, linestyle='--'))
        # Current Target
        ax.scatter(*current_target, color='darkorange', label='Low Level Target Point', zorder=5)
        # Cylinder
        for circle in circles:
        ...
        if self._near_mode:
            # 在绕行中心周围绘制两个圆环
            r_min = self.max_detect_dis
            r_max = self.max_detect_dis * 2
            roll_center = self._roll_center

            # r_min 圆
            ax.add_patch(plt.Circle(
                roll_center, r_min,
                color='red', fill=False, linestyle='--', linewidth=1.5, alpha=0.7, label="r_min"
            ))

            # r_max 圆
            ax.add_patch(plt.Circle(
                roll_center, r_max,
                color='red', fill=False, linestyle='--', linewidth=1.5, alpha=0.7, label="r_max"
            ))
        ...
        ax.legend()
        ax.set_title(f"Moving Trajectory Steps:{self.step_counter}")
