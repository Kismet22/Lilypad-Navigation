...
model_path = "./test_models/pressure_3/0801_id16.pth"

# test_path = "./train_data_2/flow_data_3.h5"
test_path = "./train_data_3/flow_data_8.h5"
...
def plot_comparison_from_batch(input_seq, other_seq, action_seq, target_seq, model, device, pmin=None, pmax=None, normalize_flag=False, target_seq_og=None, _is_pos_norm=True, _is_acc_norm=False):
    ...
    for i in range(in_dim):
        ...
        axs1[i].set_ylim([data_min - margin, data_max + margin])
    ...
    fig1.suptitle(f"Historical Input Pressure (Dim={in_dim})")
    plt.tight_layout()
    plt.show()
    ...
    # === Agent State ===
    fig2, axs2 = plt.subplots(agent_dim, 1, figsize=(12, agent_dim * 2), sharex=True)
    if agent_dim == 1:
        axs2 = [axs2]

    if _is_pos_norm:
        # 反归一化位置：维度0乘以160，维度1乘以60
        target_seq_np[:, 0] *= 160
        target_seq_np[:, 1] *= 60
        predicted_output_np[:, 0] *= 160
        predicted_output_np[:, 1] *= 60
    ...
    for i in range(agent_dim):
        ...
        margin = max(0.05 * (combined.max() - combined.min()), 0.05)
        axs2[i].set_ylim([combined.min() - margin, combined.max() + margin])

        axs2[i].set_ylabel(f"Agent {i}")
        axs2[i].grid(True)
    ...
    fig2.suptitle(f"Agent State (Position & Speed) Predicted vs. Ground Truth")
    plt.tight_layout()
    plt.show()

    # === Pressure ===
    fig3, axs3 = plt.subplots(pressure_dim, 1, figsize=(12, pressure_dim * 2), sharex=True)
    if pressure_dim == 1:
        axs3 = [axs3]

    for i in range(pressure_dim):
        idx = i + agent_dim
        axs3[i].plot(range(T_pred), target_seq_np[:, idx], label="Target (GT)", color='green', linewidth=2)
        axs3[i].plot(range(T_pred), predicted_output_np[:, idx], label="Predicted", color='red', linestyle='--', linewidth=2)

        combined = np.concatenate([target_seq_np[:, idx], predicted_output_np[:, idx]])
        margin = max(0.05 * (combined.max() - combined.min()), 0.05)
        axs3[i].set_ylim([combined.min() - margin, combined.max() + margin])

        axs3[i].set_ylabel(f"Pressure {i}")
        axs3[i].grid(True)
        axs3[i].xaxis.set_major_locator(plt.MaxNLocator(integer=True))

    axs3[-1].set_xlabel("Prediction Step")
    axs3[0].legend(loc='upper right', ncol=2)
    fig3.suptitle(f"Pressure Predicted vs. Ground Truth")
    plt.tight_layout()
    plt.show()
...
def sample_test_batch_from_existing(source_path, input_len=10, pred_len=10, ep_idx=0, t_start=20, device='cpu', _is_normalize=False, _is_pos_norm=True, _is_acc_norm=False):
    """
    ...
    # --- 裁剪 ---
    input_pressure_seq = np.clip(pressure[t_start: t_start + input_len], -10.0, 10.0)  # (input_len, 8)
    input_position_seq = position[t_start: t_start + input_len]  # (input_len, 3)
    input_speed_seq = np.clip(speed[t_start: t_start + input_len], -2.5, 2.5) # (input_len, 3)
    input_acc_seq = acc[t_start: t_start + input_len] # (input_len, 3)

    target_pressure_seq = np.clip(pressure[t_start + input_len: t_start + total_len], -10.0, 10.0)  # (pred_len, 8)
    target_position_seq = position[t_start + input_len: t_start + total_len]  # (pred_len, 3)
    target_speed_seq = np.clip(speed[t_start + input_len: t_start + total_len], -2.5, 2.5)  # (pred_len, 3)

    input_seq = input_pressure_seq # (input_len, 8)
    action_seq = acc[t_start + input_len: t_start + total_len] # (pred_len, 3)
    target_seq = np.concatenate([target_position_seq, target_speed_seq, target_pressure_seq], axis=-1)  # (pred_len, 14)
...
def test_main(_ep_id, _t_start):
    ...
    other_seq_np = other_data.cpu().numpy()[0]        # (T_in, 9)
    target_seq_np = output_data.cpu().numpy()[0]      # (T_pred, 14)

    print("======= Input Pressure Sequence (History) =======")
    for t in range(input_seq_np.shape[0]):
        print(f"t={t}: {np.round(input_seq_np[t], 4)}")

    print("======= Input Agent Sequence (History) =======")
    for t in range(other_seq_np.shape[0]):
        print(f"t={t}: {np.round(other_seq_np[t], 4)}")

    print("\n======= Target Sequence (Ground Truth) =======")
    for t in range(target_seq_np.shape[0]):
        agent_info = target_seq_np[t, :6]
        pressure_info = target_seq_np[t, 6:]
        print(f"t={t}: Agent Info: {np.round(agent_info, 4)} | Pressure: {np.round(pressure_info, 4)}")
    ...
    predicted_output_np = predicted_output[0]         # (T_pred, 14)

    print("\n======= Predicted Sequence =======")
    for t in range(predicted_output_np.shape[0]):
        agent_info = predicted_output_np[t, :6]
        pressure_info = predicted_output_np[t, 6:]
        print(f"t={t}: Agent Info: {np.round(agent_info, 4)} | Pressure: {np.round(pressure_info, 4)}")

    # 计算并打印逐步误差和全局平均误差
    errors = np.abs(predicted_output_np - target_seq_np)    # (T_pred, 14)
