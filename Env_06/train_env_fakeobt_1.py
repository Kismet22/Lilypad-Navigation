...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
        ...
        self.fake_circles = [
            {"center": (210, 64), "radius": self.window_r/2},
        ]
        #################################################
        ...
        # self.max_detect_dis = 18
        # self._old_dis_2_barricade = 18
        ############################
    ...
    def step(self, action):
        ...
        for circle in self.fake_circles:
        ...
        if self._pos_normalize:
            ...
            _distance_state = d/self.max_detect_dis
        else:
        ...
        if self.is_out_of_bounds():
            ...
            if self.is_in_circle(self.fake_circles):
                ...
                # else:
                #     _reward = -self.t_step_max * 10 * self.dt
                ...
                _truncated = True
                # _reward = -self.t_step_max * 5 * self.dt
                _reward = 0
                ...
                # action_punish = -np.linalg.norm(self.action - self.old_action)
                ########################

                ########################
                # mixed reward
                # alpha = np.clip(d / self.max_detect_dis, 0.0, 1.0)
                # target_weight = 1.0 + 9.0 * (1 - alpha)
                # _reward = -100 * self.dt + target_weight * target_reward + avoid_penalty + angle_reward + roll_swich_punish
        ...
        # TODO:dim=19
        self.state = np.hstack([state_1, barricade_state, direction, angle_norm, _distance_state]).astype(np.float32)
    ...
    def sample_safe_start_and_target(self, margin=15, max_try=100):
        """
        从训练区域采样安全的起点和终点：避开虚拟圆柱区域
        """
        x_min = self.train_zone["x_min"]
        x_max = self.train_zone["x_max"]
        y_min = self.train_zone["y_min"]
        y_max = self.train_zone["y_max"]
        
        x_center = (x_min + x_max) / 2

        for _ in range(max_try):
            # 尝试采样右侧起点
            start_x = np.random.uniform(x_center + margin, x_max - margin)
            start_y = np.random.uniform(y_min + margin, y_max - margin)
            start_point = np.array([start_x, start_y])

            if self._is_in_circles(start_point):
                continue

            # 尝试采样左侧终点
            target_x = np.random.uniform(x_min + margin, x_center - margin)
            target_y = np.random.uniform(y_min + margin, y_max - margin)
            target_point = np.array([target_x, target_y])

            if self._is_in_circles(target_point):
                continue

            return start_point, target_point
        # 如果多次尝试失败，返回默认点
        print("警告：采样失败，返回默认起点终点")
        return np.array([x_max - margin, y_min + margin]), np.array([x_min + margin, y_max - margin])
    ...
    def reset(self):
        ...
        # 在 reset 中更新虚拟圆柱的位置
        fake_y = random.uniform(54, 68)  # 或者 random.randint(54, 68) 如果想要整数
        self.fake_circles = [
        {"center": (210, fake_y), "radius": self.window_r / 2}
        ]
        # 打印坐标
        print(f"虚拟圆柱位置: center = (210, {fake_y:.2f})")
        ...
        self.start_position, self.target_position = self.sample_safe_start_and_target()
        ########################
        ...
        _barricade_states = []
        # for circle in self.circles[:3]:
        for circle in self.fake_circles[:3]:
            dx, dy = np.array(circle["center"]) - np.array(self.agent_pos)
            distance = np.linalg.norm([dx, dy])
            _barricade_states.append((distance, dx, dy, circle["center"]))
    ...
    def is_out_of_bounds(self):
        ...
        for x, y in rotated_vertices:
            ...
            if not (x_min + _delta <= x <= x_max - _delta and y_min + _delta <= y <= y_max - _delta):
