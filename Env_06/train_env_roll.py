...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _plot_flow=False, _proccess_flow=False, _random_range = 56, _flow_range=16, _init_flow_num=0, _pos_normalize=True):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        #######################################################
        # Sim Settings
        self.center_default = np.array([220, 64])
        self.center = np.array([220, 64])
    ...
    def step(self, action):
        ...
        ########################################################################################
        center_dx, center_dy = np.array(self.center) - np.array(self.agent_pos)

        ########################
        if self._pos_normalize:
            closest_dx = center_dx/self.max_detect_dis
            closest_dy = center_dy/self.max_detect_dis
        else:
            closest_dx = center_dx
            closest_dy = center_dy
        barricade_state = [closest_dx, closest_dy]
        ########################
        _dis_2_barricade = np.linalg.norm(self.agent_pos - self.center)
        self._old_dis_2_barricade = _dis_2_barricade
        if self._pos_normalize:
            _state_dis_2_barricade = _dis_2_barricade/self.max_detect_dis
        else:
            _state_dis_2_barricade = _dis_2_barricade

        # angle guidance
        obstacle_pos = np.array(self.center, dtype=np.float32)
        ...
        _lateral_error = abs(_dis_2_barricade - self.max_detect_dis)

        # Step Reward
        if _dis_2_barricade < 12 or _dis_2_barricade > 36:
            _truncated = True
            _reward = -100
            print(colored(f"**** Episode Finished At Step {self.step_counter} **** Large Lateral Error.", 'red'))
        else:
            if self.step_counter >= self.t_step_max or self.done:
                _truncated = True
                _reward = -100
                print(colored("**** Episode Finished **** Reaches Env Time limit.", 'blue'))
            elif abs(angle_norm) < 0.05  and abs(_lateral_error) < 4:
                _terminated = True
                _reward = 100
                print(colored(f"**** Episode Finished At Step {self.step_counter}  **** SUCCESS.", 'green'))
            elif self.old_direction != direction:
                    _truncated = True
                    _reward = -100
                    print(colored(f"**** Direction Switch: {self.old_direction} -> {direction} ****", 'yellow'))
            else:
                _reward = -10 * self.dt + 500 * (self.old_angle - angle_norm) * abs(self.old_direction) - _lateral_error**2 + 0.5 * (self.d_2_target - d)
        ...
        # TODO:dim=19
        self.state = np.hstack([state_1, barricade_state, _state_dis_2_barricade, direction, angle_norm]).astype(np.float32)
    ...
    def _random_start_end(self):
        epsilon = 0.05
        while True:
            theta_start = np.random.uniform(-np.pi / 3, np.pi / 3)
            theta_end = np.random.uniform(2 * np.pi / 3, 4 * np.pi / 3)
            angle_diff = abs((theta_end - theta_start + 2 * np.pi) % (2 * np.pi))
            if abs(angle_diff - np.pi) > epsilon * np.pi:
                break
        r = 24
        x_c = self.center[0]
        y_c = self.center[1]
        x_start = x_c + r * np.cos(theta_start)
        y_start = y_c + r * np.sin(theta_start)
        x_end = x_c + r * np.cos(theta_end)
        y_end = y_c + r * np.sin(theta_end)
    ...
    def reset(self):
        ...
        self.center = self.generate_point_outside_circles(self.center_default, self.random_range)
        self.start_position, self.target_position = self._random_start_end()
        ########################
