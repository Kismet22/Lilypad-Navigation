...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
        ...
        # Action Space
        self.action_dim = 3
        low = np.array([-20.0, -20, -5], dtype=np.float32)
        high = np.array([20.0, 20, 5], dtype=np.float32)
        self.low = low
        self.high = high
    ...
    def step(self, action):
        ...
        # step_1 = float(action[0])
        # step_2 = float(action[1])
        # action_json = {"v1": -20, "v2": step_1, "v3": step_2}
        ...
        close_d = min(_dis_2_barricade_array[closest_index], self.max_detect_dis)
        _dis_ratio = (self.max_detect_dis - close_d)/12
        ########################################################################################
        ...
        if self.is_out_of_bounds():
            ...
            if self.is_in_circle(self.circles):
                ...
                #####################################################################################################                
                # mask proccess
                mask = _dis_2_barricade_array < self.max_detect_dis
                _weight = 25
                _ratio = 6
                # avoid_penalty = -_weight * (1 + np.sum(np.exp(-(_dis_2_barricade_array[mask] - 12) / _ratio)))
                avoid_penalty = -_weight * (np.sum(np.exp(-(_dis_2_barricade_array[mask] - 12) / _ratio)))

                ########################
                _reward = -10 * self.dt - 10 * (d - self.d_2_target) + avoid_penalty
                ########################

                ########################
                # if self.d_2_target < self.max_detect_dis:
                #     # navigation only
                #     _reward = -10 * self.dt - 25 * (d - self.d_2_target)
                # else:
                #     # navigation & obstacle avoidence
                #     _reward = -10 * self.dt - 10 * (d - self.d_2_target) + avoid_penalty
                ########################
                #####################################################################################################
