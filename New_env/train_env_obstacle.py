    ...
    def step(self, action):
        ...
        if self.is_out_of_bounds():
            ...
            if self.is_in_circle(self.circles):
                ...
                # else:
                    # _reward = -50 * self._collision_count
                # _reward = -50 * self._collision_count
                ...
                # mask proccess
                # 0404_2
                # mask = _dis_2_barricade_array < 24   
                mask = _dis_2_barricade_array < 32

                # TODO: _weight=60 _ratio=6时,策略太保守
                _weight = 30
                _ratio = 6
                avoid_penalty = -np.sum(_weight* np.exp(-(_dis_2_barricade_array[mask] - 12) / _ratio))
                # avoid_penalty = -np.sum(self._old_dis_2_barricade - _dis_2_barricade_array)
        ...
        # TODO:dim=17
        self.state = np.hstack([state_1, _dis_2_barricade_array]).astype(np.float32)
        # TODO:dim=20
