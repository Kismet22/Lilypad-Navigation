...
class foil_env:
    ...
    def step(self, action):
        ...
        # keep [dx1, dy1]
        # barricade_state = np.array([_barricade_states[0][1], _barricade_states[0][2]], dtype=np.float32)
        closest_distance = _barricade_states[0][0]

        # keep [dx1, dy1, dx2, dy2, dx3, dy3]
        barricade_state = np.array(
            [x[1] for x in _barricade_states] + [x[2] for x in _barricade_states],
            dtype=np.float32
        ).reshape(-1, order='F').flatten()
        ...
        if self.is_out_of_bounds():
            ...
            if self.is_in_circle(self.circles):
                ...
                if closest_distance < self.max_detect_dis:
                    avoid_penalty = -_weight* (np.exp(-(closest_distance - self.max_detect_dis) / _ratio))
                else:
                    avoid_penalty = 0
                ...
                # Direction Switch Punish
                if self.old_direction != 0 and direction !=0 and self.old_direction != direction:
                    print(colored(f"**** Direction Switch: {self.old_direction} -> {direction} ****", 'red'))
                    direction_penalty = -500
                else:
                    direction_penalty = 20 * (self.old_angle - angle_norm) * abs(direction)
        ...
        # print(f"Min Distance:{self.d_2_target_min}")
        self.reward = _reward
        self._old_dis_2_barricade = _dis_2_barricade_array
        ...
        # self.state = state_1
        # TODO:dim=18
        self.state = np.hstack([state_1, barricade_state, angle_norm, direction]).astype(np.float32)
    ...
    def reset(self):
        ...
        # only keep [dx1, dy1, dx2, dy2, dx3, dy3]
        barricade_state = np.array(
            [x[1] for x in _barricade_states] + [x[2] for x in _barricade_states],
            dtype=np.float32
        ).reshape(-1, order='F').flatten()
