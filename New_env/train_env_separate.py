...
class foil_env:
    def __init__(self, config=None, info='', local_port=None, network_port=None,
             ...
             _plot_flow=False, _proccess_flow=False, _random_range = 56, _flow_range=32, _init_flow_num=0):
        """
        Basic Info:
        - Expansion Factor: 16
        ...
        self.observation_dim_nav = 14
        self.observation_dim_obt = 16
        self.state_nav = np.zeros(self.observation_dim_nav)
        self.state_obt = np.zeros(self.observation_dim_obt)
        self.observation_space_nav = Box(low=-1e6, high=1e6, shape=[self.observation_dim_nav])
        self.observation_space_obt = Box(low=-1e6, high=1e6, shape=[self.observation_dim_obt])
        ...
        # Sim Time
        self.step_counter_nav = 0
        self.step_counter_obt = 0
        self.nav_flag = True
        self.obt_flag = True
        ...
        self.closest_obstacle_pos = []
        # self.max_detect_dis = 32
    ...
    def step(self, action):
        ...
        _reward = 0
        _roll_punish = 0
        state_1 = self.state_nav
        state_2 = self.state_obt
        if self.nav_flag:
            self.step_counter_nav += 1
        if self.obt_flag:
            self.step_counter_obt += 1
        ...
        if self.nav_flag:
            # obstacle distance
            _dis_2_barricade_array = []
            _barricade_states = []
            for circle in self.circles[:3]:
                dx, dy = np.array(circle["center"]) - np.array(self.agent_pos)
                distance = np.linalg.norm([dx, dy])
                # normalize
                dx, dy = dx / self.max_detect_dis, dy / self.max_detect_dis
                # exceed max detect distance
                if distance > self.max_detect_dis:
                    distance = self.max_detect_dis
                    dx, dy = 1.0, 1.0
                _barricade_states.append((distance, dx, dy))
                _dis_2_barricade_array.append(distance)
            ...
            _dis_2_barricade_array = np.array(_dis_2_barricade_array, dtype=np.float32)
            self._old_dis_2_barricade = _dis_2_barricade_array
            # closest obstacle
            closest_index = np.argmin(_dis_2_barricade_array)
            obstacle_pos = np.array(self.circles[closest_index]["center"], dtype=np.float32)
            self.closest_obstacle_pos = obstacle_pos
            agent_pos = np.array(self.agent_pos, dtype=np.float32)
            target_pos = np.array(self.target_position, dtype=np.float32)
            if _dis_2_barricade_array[closest_index] >= self.max_detect_dis:
                angle_rad = np.pi
                direction = 0
            else:
            ...
            # Use self.closest_obstacle_pos
            agent_pos = np.array(self.agent_pos, dtype=np.float32)
            target_pos = np.array(self.target_position, dtype=np.float32)
            obstacle_pos = self.closest_obstacle_pos
            vec_to_obstacle = obstacle_pos - agent_pos
            closest_distance = np.linalg.norm(vec_to_obstacle)
            closest_dx, closest_dy = vec_to_obstacle / self.max_detect_dis
            vec1 = agent_pos - obstacle_pos
            vec2 = target_pos - obstacle_pos
            unit_vec1 = vec1 / (np.linalg.norm(vec1) + 1e-8)
            unit_vec2 = vec2 / (np.linalg.norm(vec2) + 1e-8)
        ...
        state_2[3], state_2[4] = closest_dx, closest_dy
        ...
        if self.nav_flag:
            ...
            if full_turns != 0:
                ...
                _roll_punish = abs(full_turns) * self._roll_punish
            self._roll_count = current_roll_count
            # Navigation Reward
            if self.is_out_of_bounds():
                _nav_fail = True
                _reward = -50
                print(colored("**** Navigation Episode Finished **** Hit Boundary.", 'red'))
            elif self.step_counter_nav >= self.t_step_max:
                _nav_fail = True
                _reward = -50
                print(colored("**** Navigation Episode Finished **** Reaches Env Time limit.", 'red'))
            elif closest_distance < self.max_detect_dis:
                _nav_switch = True
                _reward = -50
                print(colored(f"**** Direction Confirm: {direction} ****", 'magenta'))
                print(colored("**** Navigation Episode Switch **** Obstacle Detect.", 'blue'))
            elif self.is_reach_target():
                _nav_success = True
                _reward = 1000
                print(colored("**** Navigation Episode Finished **** SUCCESS.", 'green'))
            else:
                _reward = -10 * self.dt - 10 * (d - self.d_2_target)
            _reward = _reward + _roll_punish
            ...
            self._roll_count = current_roll_count
            # Obstacle Reward
            if self.is_in_circle(self.circles) or (self.max_detect_dis - closest_distance) < 12:
                self._collision_count += 1
                #####################################################################################################  
                # end episode
                _obt_fail = True
                _reward = -50
                print(colored("**** Obstacle Episode Finished **** Collision.", 'red'))
                #####################################################################################################
            elif self.step_counter_obt >= self.t_bot_max:
                _obt_fail = True
                _reward = -50
                print(colored("**** Obstacle Episode Finished **** Reaches Env Time limit.", 'red'))
            elif abs(closest_distance - self.max_detect_dis) < 12:
                _obt_fail = True
                _reward = -50
                print(colored("**** Obstacle Episode Finished **** Too Far.", 'red'))
            elif self.old_direction != 0 and direction !=0 and self.old_direction != direction:
                _obt_fail = True
                _reward = -50
                print(colored("**** Obstacle Episode Finished **** Wrong Direction.", 'red'))
            elif abs(angle_rad) <= 1e-2:
                _obt_switch = True
                _reward = 50
                print(colored("**** Obstacle Episode Switch **** Switch.", 'blue'))
            else:
                avoid_penalty = -(closest_distance - self.max_detect_dis)**2
                direction_penalty = 100 * (self.old_angle - angle_rad) * abs(direction)        
                _reward = -10 * self.dt + avoid_penalty + direction_penalty
            _reward = _reward + _roll_punish
        else:
        ...
        self.old_angle = angle_rad
        self.old_direction = direction
        self.reward = _reward
        self._old_dis_2_barricade = _dis_2_barricade_array
        # dim = 14
        self.state_nav = state_1
        # dim = 16
        self.state_obt = np.hstack([state_2, angle_rad, direction]).astype(np.float32)
        ...
        if self.include_flow:
            ...
            if self._plot_flow:
                ...
                _save = _obt_fail or _nav_fail or _nav_success
            ...
            if self._proccess_flow:
                ...
                return self.state_nav, self.state_obt, self.nav_flag, self.obt_flag, _nav_success, _nav_fail, _nav_switch, _obt_fail, _obt_switch, _info, flow_input
        return self.state_nav, self.state_obt, self.nav_flag, self.obt_flag, _nav_success, _nav_fail, _nav_switch, _obt_fail, _obt_switch, _info,
    ...
    def reset(self):
        ...
        return self.state_nav, self.state_obt, self.nav_flag, self.obt_flag, _info
